...
function info(msg) { console.log(`${GREEN}[context-sync]${NC} ${msg}`); }
function warn(msg) { console.log(`${YELLOW}[context-sync]${NC} ${msg}`); }
function error(msg) { console.error(`${RED}[context-sync]${NC} ${msg}`); process.exit(1); }
  ...
  while (i < rest.length) {
    switch (rest[i]) {
      ...
      case '--tool':
        ...
        break;
      case '--help': case '-h':
        console.log(`Usage: context-sync [options]

Options:
  --bundle full|scan-only|enforce-only   Skill bundle (default: full)
...
function installClaudeCode(targetDir, selectedSkills) {
  ...
  if (fs.existsSync(claudeMdPath)) {
    fs.appendFileSync(claudeMdPath, '\n\n# --- ContextSync Rules (appended by context-sync) ---\n');
...
function installCursor(targetDir, selectedSkills) {
  ...
  const rulesContent = fs.readFileSync(path.join(SCRIPT_DIR, 'CLAUDE.md'), 'utf8');
  fs.writeFileSync(path.join(rulesDir, 'context-sync-rules.mdc'), rulesContent);
...
function main() {
  ...
  if (!BUNDLES[args.bundle]) {
    error(`Unknown bundle: ${args.bundle}. Use: full, scan-only, enforce-only`);
  }
